...
def is_abundant(check_number):
    if sum_set(proper_divisors(check_number)) > check_number:
        return True
    else:
        return False
...
def main():
    limit = 28123
    sum_to_limit = int(limit * (limit + 1) / 2)
    result = sum_to_limit
    ...
    for abn_a in abundant_numbers:
        for abn_b in abundant_numbers:
            if (abn_a + abn_b) > limit:
                break
            else:
                abundant_sums.add(abn_a + abn_b)
    result = sum_to_limit - sum_set(abundant_sums)
    print(result)
